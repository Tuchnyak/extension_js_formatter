...
Сейчас в репозитории есть только `SPEC.md` — подробная пошаговая спецификация офлайн JSON Viewer (Chrome-расширение, MV3). Кода, `package.json` и git-репозитория пока нет. `SPEC.md` — источник истины; перед работой читать его целиком (он на русском).
...
Скрипты, которые должны быть в `package.json`: `npm run dev` (wxt), `build`, `zip`, `compile` (`tsc --noEmit`), `check` (`biome check .`); с v2 — `npm run test`.
